...
const JobCard = ({ job }) => {
  ...
  return (
    <div className='border p-6 shadow rounded'>
      ...
      <h4 className='font-medium text-xl mg-medium'>{job.title}</h4>
      ...
      <div className='mt-4 flex gap-4 text-sm'>
        ...
        <button onClick={()=> {navigate(`/apply-job/${job._id}`); scrollTo(0,0)}}  className='text-gray-500 border border-gray-500 rounded cursor-pointer px-4 py-2'>Learn More</button>
      </div>
    </div>
  )
}
